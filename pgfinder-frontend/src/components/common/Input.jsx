function Input({ label, className = '', error, ...props }) {
  return (
    <label className={`block text-sm text-slate-200 ${className}`}>
      ...
      <input
        className="w-full rounded-3xl border border-slate-700 bg-slate-950/70 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-accent-400 focus:ring-2 focus:ring-accent-400/20"
        {...props}
      />
